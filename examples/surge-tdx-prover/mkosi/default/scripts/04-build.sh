...
if ! ([ -d "$BUILDDIR/tdxs-master" ] && [ "$(ls -A "$BUILDDIR/tdxs-master" 2>/dev/null)" ]); then git clone --depth=1 -b master https://github.com/NethermindEth/tdxs "$BUILDROOT/build/tdxs" && mkosi-chroot bash -c 'cd /build/tdxs && make sync-constellation && go build -trimpath -ldflags "-s -w -buildid=" -o ./build/tdxs ./cmd/tdxs/main.go' && mkdir -p "$BUILDDIR/tdxs-master" && install -D -m 0755 "$BUILDROOT/build/tdxs/build/tdxs" "$BUILDDIR/tdxs-master"/tdxs; fi && install -D -m 0755 "$BUILDDIR/tdxs-master"/tdxs "$DESTDIR/usr/bin/tdxs"
if ! ([ -d "$BUILDDIR/key-generation-main" ] && [ "$(ls -A "$BUILDDIR/key-generation-main" 2>/dev/null)" ]); then git clone --depth=1 -b main https://github.com/NethermindEth/nethermind-tdx "$BUILDROOT/build/key-generation" && mkosi-chroot bash -c 'cd /build/key-generation/init && go build -trimpath -ldflags "-s -w -buildid=" -o ./build/key-generation ./cmd/main.go' && mkdir -p "$BUILDDIR/key-generation-main" && install -D -m 0755 "$BUILDROOT/build/key-generation/init/build/key-generation" "$BUILDDIR/key-generation-main"/key-generation; fi && install -D -m 0755 "$BUILDDIR/key-generation-main"/key-generation "$DESTDIR/usr/bin/key-generation"
if ! ([ -d "$BUILDDIR/disk-encryption-main" ] && [ "$(ls -A "$BUILDDIR/disk-encryption-main" 2>/dev/null)" ]); then git clone --depth=1 -b main https://github.com/NethermindEth/nethermind-tdx "$BUILDROOT/build/disk-encryption" && mkosi-chroot bash -c 'cd /build/disk-encryption/init && go build -trimpath -ldflags "-s -w -buildid=" -o ./build/disk-encryption ./cmd/main.go' && mkdir -p "$BUILDDIR/disk-encryption-main" && install -D -m 0755 "$BUILDROOT/build/disk-encryption/init/build/disk-encryption" "$BUILDDIR/disk-encryption-main"/disk-encryption; fi && install -D -m 0755 "$BUILDDIR/disk-encryption-main"/disk-encryption "$DESTDIR/usr/bin/disk-encryption"
if ! ([ -d "$BUILDDIR/secret-delivery-main" ] && [ "$(ls -A "$BUILDDIR/secret-delivery-main" 2>/dev/null)" ]); then git clone --depth=1 -b main https://github.com/NethermindEth/nethermind-tdx "$BUILDROOT/build/secret-delivery" && mkosi-chroot bash -c 'cd /build/secret-delivery/init && go build -trimpath -ldflags "-s -w -buildid=" -o ./build/secret-delivery ./cmd/main.go' && mkdir -p "$BUILDDIR/secret-delivery-main" && install -D -m 0755 "$BUILDROOT/build/secret-delivery/init/build/secret-delivery" "$BUILDDIR/secret-delivery-main"/secret-delivery; fi && install -D -m 0755 "$BUILDDIR/secret-delivery-main"/secret-delivery "$DESTDIR/usr/bin/secret-delivery"
